...
export async function getBusinessesByStatus(status) {
  // Hot first, then warm, then cold leads that have a known email (found via enrichment)
  ...
  // Cold leads with a confirmed email are worth building for — no domain urgency signal
  // but enrichment found a real address we can reach
  const { data: cold, error: e3 } = await supabase
    .from('businesses')
    .select('*')
    .eq('pipeline_status', status)
...
// Log external API usage for cost tracking and the weekly finance report.
// cost_usd: estimated cost; null = free or unknown.
export async function logApiUsage(api, calls, costUsd = null, { agent = null, notes = null } = {}) {
  const { error } = await supabase
    .from('api_usage')
    .insert({ api, agent, calls, cost_usd: costUsd, notes });
  if (error) console.error(`Failed to log api_usage (${api}):`, error.message);
}

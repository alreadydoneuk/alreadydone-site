...
const EMAIL_JUNK_DOMAINS = new Set([
  ...
  'website.com', 'sentry.io', 'cloudflare.com', 'wixpress.com',
...
function assessConfidence(text, businessName, phone, postcode) {
  ...
  const nameFound = text.toLowerCase().includes(businessName.toLowerCase().split(' ')[0]);

  if (nameFound && postcodeFound) return 'medium';
  return 'low';
}
...
async function searchByDomain(domain, businessName, phone, postcode) {
  ...
  for (const item of results) {
    ...
    if (snippetEmails.length) {
      const confidence = assessConfidence(combinedText, businessName, phone, postcode);
      if (confidence !== 'low') {
    ...
    if (!isBlockedDomain(item.link)) {
      ...
      const pageConfidence = assessConfidence(html, businessName, phone, postcode);
      if (pageEmails.length && pageConfidence !== 'low') {
...
async function searchByNameEmail(businessName, location, phone, postcode) {
  ...
  for (const item of results) {
    ...
    const snippetEmails = extractEmails(combinedText);
    if (snippetEmails.length && confidence !== 'low') {

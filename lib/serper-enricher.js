...
const AGGREGATOR_PATTERNS = [
  ...
  /yellowpages\./, /brownbook\.net/, /scoot\.co\.uk/, /cyclex\.co\.uk/, /cylex-uk\.co\.uk/, /fyple\.co\.uk/, /nailsalondirect\.com/,
  ...
  /setmore\.com/, /nearcut\.com/, /menufyy\.com/, /mybistro\.online/,
  /weeblyte\.com/, /wheree\.com/, /open-closed\.net/, /goto-where\.com/,
  /restaurants-world\.net/, /com-place\.com/, /menu-world\.com/, /restaurant-guru\.com/,
  /naturaltherapypages\.co\.uk/, /alivenetwork\.com/, /gigsalad\.com/,
];
...
// ─── Website detection (exported for use at enrichment time) ─────────────────

// Searches Serper for the business and returns their own website URL if found.
// Used as the first step in enrichment — we want to PROVE they don't have a site
// before treating them as a prospect, not assume they don't.
export async function findBusinessWebsite(business) {
  const city = (business.location || '').split(',')[0].trim();
  const results = await serperSearch(`"${business.name}" "${city}"`);
  if (!results.length) return null;
  const found = detectExistingWebsite(results, business);

...
- Frame it as genuinely helpful information first, offer second`;
    ...
    hook = `Their website (${b.domain}) is listed on their Google profile but isn't displaying properly — the site loads but something is wrong with it, so visitors aren't seeing what they should.`;
    context = `- The domain and server are fine but the site itself is broken — could be an outdated CMS, broken theme, missing content, or similar
- Customers landing on it from Google get a poor or confusing experience
- They may not be aware — it might look fine on their own device (cached) but broken for new visitors
- Frame it as a helpful heads up: I noticed your site has an issue, and rather than just flag it I went ahead and built you a clean replacement
- Don't speculate about what specifically is broken — just say it isn't displaying right for visitors`;
...
- Frame it as: I noticed you didn't have a site yet, so I built a preview`;
  ...
  const previewLine = previewUrl
    ? `I have built a live preview website for them — tell them they can see it at the link below this email (do NOT paste the URL into the body text — just write "the link below" or "the preview below")`

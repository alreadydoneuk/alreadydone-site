...
// Extracts the site's existing CSS to match styles exactly.
export async function generateExtraPageSection(existingHtml, pageSpec, business) {
  ...
  const message = await callWithRetry({
    model: HAIKU_MODEL,
    max_tokens: 2000,
    system: `You are adding a new section to an existing single-page business website. Match the existing design precisely. Return ONLY the <section> element — no explanation, no surrounding HTML.`,
    messages: [{
      role: 'user',
...
- id attribute matching the section type (e.g. id="about", id="gallery", id="menu")
- All styles inline or in a <style> block inside the section — no external CSS
- Design that matches the existing site's colours, fonts, and layout conventions

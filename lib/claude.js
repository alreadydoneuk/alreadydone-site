...
The emails must feel genuinely human — like a helpful neighbour passing on something useful, not a salesperson.
Never use bullet points, corporate language, or salesy phrases.
Write in first person. Target 120–140 words — tight and complete. Do not trail off or end mid-thought.`;
  ...
  const isBroken = b.website_status === 'broken';
  ...
  if (isExpired) {
    const daysAgo = b.whois_expiry_date
      ...
      : 1;
    hook = `Their domain (${b.domain}) expired ${daysAgo === 1 ? 'yesterday' : `${daysAgo} days ago`} — their website is completely offline and their Google listing leads to a dead end.`;
    context = `- Open with the fact that the site is down — they may not know
- Every day offline costs them enquiries — convey this without being alarmist
- The offer is getting back online today, on a new domain, for £${b.price || 99}
- Don't tell them to just renew — frame it as a fresh start that's ready to go now`;
  } else if (isExpiringSoon) {
    hook = `Their domain (${b.domain}) expires tomorrow — after that their website AND their custom email address both go dark.`;
    context = `- Lead with the email angle — they almost certainly don't realise their @${b.domain.replace(/^www\./, '')} email is at risk, not just the website
- Frame it as passing on useful information: I happened to notice, I built a preview, we can sort it before the deadline
- Do NOT tell them to just renew their domain — the offer is a proper new site on a fresh domain for £${b.price || 99}`;
    ...
    hook = `Their website (${b.domain}) is on their Google listing but isn't loading right now — anyone who clicks it gets an error.`;
    context = `- They almost certainly don't know — open with the problem as a genuine heads-up
- Then: I didn't just flag it, I went ahead and built a replacement
- Helpful information first, offer second`;
  } else if (isBroken) {
    hook = `Their website (${b.domain}) loads but isn't displaying properly — visitors aren't seeing what they should.`;
    context = `- They may not realise — it can look fine on their own device (cached) but broken for new visitors
- Open with the heads-up, then: rather than just flag it I built a clean replacement
- Don't speculate about what's broken — just that it isn't showing right`;
  } else if (isComingSoon) {
    hook = `Their domain (${b.domain}) has a "coming soon" page — they started a website and never finished it.`;
    context = `- They clearly meant to have a site — open with the observation that it's been sitting there
- Frame it as: I noticed you got started but never finished it, so I finished it for you
- Tone: casual, like a colleague who spotted something and just got on with it`;
  } else {
    hook = `Their domain (${b.domain}) is registered but just shows a parked page — no real website.`;
    context = `- Open with what they're missing: potential customers finding a blank page instead of a business
- Frame it as: I noticed you didn't have a site, so I built one to show you what it could look like`;
  }

  const previewLine = previewUrl
    ? `There is a live preview website ready for them — tell them to click the link below this email to see it (do NOT write the URL in the body — just say "the link below" or "take a look below")`
    : `There is a preview website built and ready for them`;

  const ratingHook = b.google_rating && b.review_count > 5
    ? `They have ${b.review_count} Google reviews averaging ${b.google_rating} stars — strong reputation. Weave this in: their customers clearly rate them, but without a working website that reputation isn't reaching anyone new.`
    : '';
...
- ${previewLine}
- The price is £${b.price || 99}. Anchor it once: agencies charge £1,000–£2,000; I can do it for £${b.price || 99} because it's already built.
- End with: if they're not interested they can ignore it and the preview comes down in two weeks. If they want it, just reply.

Structure (4 beats, flow naturally — do NOT use headers or lists):
1. Hook — open with the specific problem or observation. Do not start by introducing yourself.
2. What I did — one sentence: I went ahead and built a site, take a look below.
3. The offer — price + anchor in one or two sentences.
4. Close — low pressure. Reply to claim it, or ignore it and it disappears.

Introduce yourself as Dean from Already Done in beat 1 or 2, briefly and naturally — not as the opener.
Sign off with just "Dean".
Do NOT include a subject line. Return only the email body.
Stay between 120–140 words. End with a complete sentence — never trail off mid-pitch.`;

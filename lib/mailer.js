...
export async function sendAutoReply({ to, subject, body, inReplyTo }) {
  ...
  const { data, error } = await resend.emails.send({
    ...
    headers: inReplyTo ? (() => { const id = inReplyTo.startsWith('<') ? inReplyTo : `<${inReplyTo}>`; return { 'In-Reply-To': id, References: id }; })() : undefined,
  });
...
export async function sendOnboardingStarted({ to, firstName, domain, emailPrefix, plan, noCustomDomain = false }) {
  const resend = getClient();
  const name = firstName || 'there';
  const hasEmail = plan === 'site_and_email' && emailPrefix && !noCustomDomain;
  ...
  const lines = [
    ...
    ...(noCustomDomain ? [] : [`  → Registering ${domain}`]),
    `  → Building your site`,
...
export async function sendOnboardingComplete({ to, firstName, domain, emailPrefix, emailPassword, plan, extraAccounts = [], noCustomDomain = false }) {
  const resend = getClient();
  const name = firstName || 'there';
  ...
  const lines = [
    ...
    `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`,
    ...(noCustomDomain
      ? [`Your hosting renews automatically one year from today.`]
...
export async function sendDomainTakenNotification({ to, firstName, domain }) {
  const resend = getClient();
  const name = firstName || 'there';
  ...
  const lines = [
    `Hi ${name},`,
    ``,
    `We've run into a problem registering ${domain} — it looks like it was just taken by someone else in the time between you choosing it and us trying to register it.`,
    ``,
    `Don't worry — your payment is safe and your site is built and ready to go.`,
    ``,
    `Could you reply to this email with an alternative domain name? For example:`,
    `  - ${domain.replace(/\.(co\.uk|com|uk)$/, '')}.uk`,
    `  - ${domain.replace(/\.(co\.uk|com|uk)$/, '')}.net`,
    `  - Or anything else you'd like to try`,
    ``,
    `We'll get it registered and live as soon as you reply. Sorry for the hassle.`,
    ``,
    `Dean`,
    `Already Done`,
  ];
  ...
  const { data, error } = await resend.emails.send({
    from: `${process.env.FROM_NAME || 'Dean'} <${process.env.FROM_EMAIL || 'dean@alreadydone.uk'}>`,
    to,
    subject: `Problem with your domain — quick fix needed`,
    text: body,
    html: plainToHtml(body, null),
  });

  if (error) throw new Error(`Resend error: ${error.message}`);
  console.log(`    Domain taken notification sent — id: ${data.id}`);
  return { messageId: data.id };
}
